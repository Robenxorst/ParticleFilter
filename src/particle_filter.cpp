...
void ParticleFilter::init(double x, double y, double theta, const double std[]) {
    ...
    // стандартные отклонения по x,y, theta
    std_x = std[0];
    std_y = std[1];
    std_theta = std[2];
...
void ParticleFilter::updateWeights(double sensor_range, const double std_landmark[],
    ...
    // Обновление веса каждой частицы,с использованием многомерного распределения Гаусса.
    // Подробнее о многомерном распределении: https://en.wikipedia.org/wiki/Multivariate_normal_distribution
    // ПРИМЕЧАНИЕ: Наблюдения(observations) даны в системе координат РОБОТА. Ваши частицы расположены
    //  в системе координат карты. Т.о. необходимо перейти от системы координат робота к системе координат карты.
    //  Это преобразование требует как вращения, так и перемещения (но не масштабирования).
    //  Ссылка на теоретический источник:
    //  https://www.willamette.edu/~gorr/classes/GeneralGraphics/Transforms/transforms2d.htm
    //   Ссылка на реализуемое уравнение:
    //   http://planning.cs.uiuc.edu/node99.html
