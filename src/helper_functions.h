...
#include <sstream>// подключение istringstream 85 строка
#include <fstream>// ifstream 73
 ...
 */
struct control_s { // структура отвечающая за линейную скорость и угловую скорость
 ...
 */
struct ground_truth { // структура отвечает за истинную позицию робота
 ...
 */
struct LandmarkObs {// структура объекта на карте
...
// функция создающая массив с рассогласованиями
// между значением из фильтра частиц(pf) и истинным значением положения робота(gt)
inline double * getError(double gt_x, double gt_y, double gt_theta, double pf_x, double pf_y, double pf_theta) {
	static double error[3];
    // берем разницу между числами по модулю
	error[0] = fabs(pf_x - gt_x);
	error[1] = fabs(pf_y - gt_y);
	error[2] = fabs(pf_theta - gt_theta);
    // возвращаем остаток от деления error[2] на 2 * pi(360 град)
	error[2] = fmod(error[2], 2.0 * M_PI);
	if (error[2] > M_PI) {
		error[2] = 2.0 * M_PI - error[2];
	}
	return error;   //возвращаем указатель на начало массива ошибок
}
 ...
 */
// считываем данные из string-файла filename и кладем их в класс Map
...
// считываем контрольные данные(сведения о текущей линейной и угловой скорости робота)
// из filename и кладем их в vector position_meas
...
// отвечает за прочтение истинных данных(ground truth)
// о расположении робота из строчки filename и перемещение их в vector gt;
inline bool read_gt_data(std::string filename, std::vector<ground_truth>& gt) {
    ...
    // аналог getnext line
		...
		single_gt.x = x;
		single_gt.y = y;
		single_gt.theta = azimuth;// угол рысканья
...
// считываем объекты на карте из файла и перемещаем их в vector observation
inline bool read_landmark_data(std::string filename, std::vector<LandmarkObs>& observations) {
	...
	while(getline(in_file_obs, line_obs)){
		...
		LandmarkObs meas; // структура объекта на карте
